...
describe('useWeather', () => {
    it(`test 'determineDayDescription' function`, () => {
        ...
        testTheDayBeforeYesterdayDate.setDate(testTheDayBeforeYesterdayDate.getDate() - 2)
        expect(result.current.determineDayDescription(testTheDayBeforeYesterdayDate)).toStrictEqual('前天')

        const testYesterdayDate = new Date()
        testYesterdayDate.setDate(testYesterdayDate.getDate() - 1)
        expect(result.current.determineDayDescription(testYesterdayDate)).toStrictEqual('昨天')

        const testTodayDate = new Date()
        expect(result.current.determineDayDescription(testTodayDate)).toStrictEqual('今天')

        const testTomorrowDate = new Date()
        testTomorrowDate.setDate(testTomorrowDate.getDate() + 1)
        expect(result.current.determineDayDescription(testTomorrowDate)).toStrictEqual('明天')

        const testTheDayAfterTomorrowDate = new Date()
        testTheDayAfterTomorrowDate.setDate(testTheDayAfterTomorrowDate.getDate() + 2)
        expect(result.current.determineDayDescription(testTheDayAfterTomorrowDate)).toStrictEqual('後天')
    })
    ...
    it(`test 'creatWeatherCardList' function`, () => {
        ...
        const selectedTaipeiWeather = result.current.weatherAllLocation.find((item) => item.locationName === '臺北市')
        expect(result.current.creatWeatherCardList(selectedTaipeiWeather)).toStrictEqual([
            {
        ...
        const selectedNewTaipeiCityWeather = result.current.weatherAllLocation.find((item) => item.locationName === '新北市')
        expect(result.current.creatWeatherCardList(selectedNewTaipeiCityWeather)).toStrictEqual([
            {
                weatherDescriptionName: '陰天',
                weatherDescriptionCode: '7',
                weatherPeriodTime: '今天下午 至 今天晚上',
